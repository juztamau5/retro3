async function register ({ registerHook, registerSetting, settingsManager, storageManager, Retro3Helpers }) {
  {
    const actionHooks = [
    ...
    for (const h of actionHooks) {
      registerHook({
        target: h,
        handler: () => Retro3Helpers.logger.debug('Run hook %s.', h)
      })
    }
    ...
    for (const h of [ 'action:activity-pub.remote-video.created', 'action:activity-pub.remote-video.updated' ]) {
      registerHook({
        ...
        handler: ({ video, videoAPObject }) => {
          Retro3Helpers.logger.debug('Run hook %s - AP %s - video %s.', h, video.name, videoAPObject.name )
        }
      })
    }
  }
  ...
  registerHook({
    ...
    handler: async (result, params) => {
      const loggedInUser = await Retro3Helpers.user.getAuthUser(params.res)
...

  {
    ...
    for (const h of filterHooks) {
      registerHook({
        target: h,
        handler: (obj) => {
          Retro3Helpers.logger.debug('Run hook %s.', h)
