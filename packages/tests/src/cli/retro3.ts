...
describe('Test CLI wrapper', function () {
  ...
  const cmd = 'node ./apps/retro3-cli/dist/retro3.js'
  ...
  describe('Manage plugins', function () {
    ...
    it('Should install a plugin', async function () {
      ...
      await cliCommand.execWithEnv(`${cmd} plugins install --npm-name retro3-plugin-hello-world`)
    })
    ...
    it('Should list installed plugins', async function () {
      ...
      expect(res).to.contain('retro3-plugin-hello-world')
    })

    it('Should uninstall the plugin', async function () {
      const res = await cliCommand.execWithEnv(`${cmd} plugins uninstall --npm-name retro3-plugin-hello-world`)

      expect(res).to.not.contain('retro3-plugin-hello-world')
    })

    it('Should install a plugin in requested version', async function () {
      this.timeout(60000)

      await cliCommand.execWithEnv(`${cmd} plugins install --npm-name retro3-plugin-hello-world --plugin-version 0.0.17`)
    })
    ...
    it('Should install a plugin in requested beta version', async function () {
      ...
      await cliCommand.execWithEnv(`${cmd} plugins install --npm-name retro3-plugin-hello-world --plugin-version 0.0.21-beta.1`)
      ...
      await cliCommand.execWithEnv(`${cmd} plugins uninstall --npm-name retro3-plugin-hello-world`)
    })
  })
