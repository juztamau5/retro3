...
Prepare PostgreSQL user so retro3 can delete/create the test databases:
 ...
 * `AKISMET_KEY`: specify an Akismet key to test akismet external retro3 plugin
...
You can also:
 - checkout only the latest logs (retro3 >= 5.0):

```bash
tail -n 100 test1/logs/retro3.log | npm run parse-log -- --level debug --files -
```

 - continuously print the latests logs (retro3 >= 5.0):

```bash
tail -f test1/logs/retro3.log | npm run parse-log -- --level debug --files -
```
...
```bash
RETRO3_E2E_PASSWORD=password npm run e2e:local
```
