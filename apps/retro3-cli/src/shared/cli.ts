...
import { getAppNumber, isTestInstance, root } from '@retroai/retro3-node-utils'
...
async function getAdminTokenOrDie (server: Retro3Server, username: string, password: string) {
...
function buildServer (url: string) {
  return new Retro3Server({ url })
}

async function assignToken (server: Retro3Server, username: string, password: string) {
