...
export function defineUploadProgram () {
  const program = new Command('upload')
    .description('Upload a video on a retro3 instance')
    .alias('up')
...
async function run (options: UploadOptions) {
  ...
  try {
    ...
    if (message.includes('413')) {
      console.error('Aborted: user quota is exceeded or video file is too big for this retro3 instance.')
    } else {
...
async function buildVideoAttributesFromCommander (server: Retro3Server, options: UploadOptions, defaultAttributes: any = {}) {
