...
export function defineAuthProgram () {
      ...
      prompt.get({
        properties: {
          url: {
            ...
            conform: value => isURLaRetro3Instance(value),
            message: 'It should be an URL (https://retro3.example.com)',
            required: true
          },
          username: {
        ...
        try {
          ...
          result.url = stripExtraneousFromRetro3Url(result.url)
  ...
  program.addHelpText('after', '\n\n  Examples:\n\n' +
    '    $ retro3 auth add -u https://peertube2.cpy.re -U "RETRO3_USER" --password "RETRO3_PASSWORD"\n' +
    '    $ retro3 auth add -u https://peertube2.cpy.re -U root\n' +
    '    $ retro3 auth list\n' +
    '    $ retro3 auth del https://peertube2.cpy.re\n'
  )
...
function isURLaRetro3Instance (url: string) {
  return url.startsWith('http://') || url.startsWith('https://')
}

function stripExtraneousFromRetro3Url (url: string) {
