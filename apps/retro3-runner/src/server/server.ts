...
import { pick, shuffle, wait } from '@retroai/retro3-core-utils'
import { Retro3ProblemDocument, ServerErrorCode } from '@retroai/retro3-models'
import { Retro3Server as Retro3ServerCommand } from '@retroai/retro3-server-commands'
...
type Retro3Server = Retro3ServerCommand & {
  runnerToken: string
  runnerName: string
...
export class RunnerServer {
  ...
  private servers: Retro3Server[] = []
  private processingJobs: { job: JobWithToken, server: Retro3Server }[] = []

  private checkingAvailableJobs = false

  private cleaningUp = false

  private readonly sockets = new Map<Retro3Server, Socket>()

  private constructor () {}

  async run () {
    logger.info('Running retro3 runner in server mode')

    await ConfigManager.Instance.load()

    for (const registered of ConfigManager.Instance.getConfig().registeredInstances) {
      const serverCommand = new Retro3ServerCommand({ url: registered.url })
  ...
  private loadServer (server: Retro3Server) {
  ...
  private unloadServer (server: Retro3Server) {
  ...
  private async checkAvailableJobs () {
    ...
    for (const server of shuffle([ ...this.servers ])) {
      try {
        ...
        const code = (err.res?.body as Retro3ProblemDocument)?.code
  ...
  private async requestJob (server: Retro3Server) {
  ...
  private async tryToExecuteJobAsync (server: Retro3Server, jobToAccept: { uuid: string }) {
