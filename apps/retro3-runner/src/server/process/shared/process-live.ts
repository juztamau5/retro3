...
import { ffprobePromise, getVideoStreamBitrate, getVideoStreamDimensionsInfo, hasAudioStream } from '@retroai/retro3-ffmpeg'
import {
...
export class ProcessLiveRTMPHLSTranscoding {
  ...
  private onUpdateError (options: {
    ...
    const type = ((err as any).res?.body as Retro3ProblemDocument)?.code
  ...
  private async updateWithRetry (payload: LiveRTMPHLSTranscodingUpdatePayload, currentTry = 1): Promise<any> {
    ...
    try {
      ...
      if ((err.res?.body as Retro3ProblemDocument)?.code === ServerErrorCode.RUNNER_JOB_NOT_IN_PROCESSING_STATE) throw err
