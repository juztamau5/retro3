...
export class IPCClient {
  ...
  async askRegister (options: {
    ...
    if (success) console.log('retro3 instance registered')
    else console.error('Could not register retro3 instance on runner server side', error)
  }
  ...
  async askUnregister (options: {
    ...
    if (success) console.log('retro3 instance unregistered')
    else console.error('Could not unregister retro3 instance on runner server side', error)
  }
  ...
  async askListRegistered () {
    ...
    if (!success) {
      console.error('Could not list registered retro3 instances', error)
      return
    }
