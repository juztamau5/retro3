...
let retro3LocalStorage: Storage
let retro3SessionStorage: Storage

function reinitStorage () {
  const instanceLocalStorage = new MemoryStorage()
  const instanceSessionStorage = new MemoryStorage()

  retro3LocalStorage = proxify(instanceLocalStorage)
  retro3SessionStorage = proxify(instanceSessionStorage)
}

try {
  retro3LocalStorage = localStorage
  retro3SessionStorage = sessionStorage
} catch (err) {
...
if (!retro3LocalStorage || !retro3SessionStorage) {
  reinitStorage()
}

export {
  retro3LocalStorage,
  retro3SessionStorage
}
