...
@Injectable()
export class Retro3RouterService {
  ...
  getRouteSetting (snapshot?: ActivatedRouteSnapshot) {
    return (snapshot || this.route.snapshot).queryParams[Retro3RouterService.ROUTE_SETTING_NAME]
  }
  ...
  setRouteSetting (value: number) {
    ...
    const queryParams = { [Retro3RouterService.ROUTE_SETTING_NAME]: value }
  ...
  silentNavigate (baseRoute: string[], queryParams: { [id: string]: string }) {
    ...
    queryParams = {
      ...
      [Retro3RouterService.ROUTE_SETTING_NAME]: routeSetting
    }
