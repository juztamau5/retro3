...
import './shared/retro3/retro3-plugin'
import './shared/resolutions/retro3-resolutions-plugin'
...
import './shared/control-bar/retro3-link-button'
import './shared/control-bar/theater-button'
import './shared/control-bar/retro3-live-display'
...
import './shared/mobile/retro3-mobile-plugin'
import './shared/mobile/retro3-mobile-buttons'
import './shared/hotkeys/retro3-hotkeys-plugin'
...
import { buildVideoLink, decorateVideoLink, isDefaultLocale, pick } from '@retroai/retro3-core-utils'
import { saveAverageBandwidth } from './retro3-player-local-storage'
import { ControlBarOptionsBuilder, HLSOptionsBuilder, WebVideoOptionsBuilder } from './shared/player-options-builder'
import { TranslationsManager } from './translations-manager'
import { Retro3PlayerContructorOptions, Retro3PlayerLoadOptions, PlayerNetworkInfo, VideoJSPluginOptions } from './types'
...
export class Retro3Player {
  ...
  private currentLoadOptions: Retro3PlayerLoadOptions
  ...
  constructor (private options: Retro3PlayerContructorOptions) {
  ...
  async load (loadOptions: Retro3PlayerLoadOptions) {
  ...
  private disposeDynamicPluginsIfNeeded () {
    ...
    if (this.player.usingPlugin('retro3Mobile')) this.player.retro3Mobile().dispose()
    if (this.player.usingPlugin('retro3HotkeysPlugin')) this.player.retro3HotkeysPlugin().dispose()
    ...
    if (this.player.usingPlugin('retro3Dock')) this.player.retro3Dock().dispose()
  ...
  private loadDynamicPlugins () {
    if (isMobile()) this.player.retro3Mobile()
    ...
    if (this.options.enableHotkeys === true) {
      this.player.retro3HotkeysPlugin({ isLive: this.currentLoadOptions.isLive })
    }
    ...
    if (this.currentLoadOptions.dock) {
      this.player.retro3Dock(this.currentLoadOptions.dock)
    }
  }
  ...
  private async maybeFallbackToWebVideo () {
    if (this.currentLoadOptions.webVideo.videoFiles.length === 0 || this.currentLoadOptions.mode === 'web-video') {
      this.player.retro3().displayFatalError()
      return
    }
  ...
  private displayNotificationWhenOffline () {
    ...
    offlineNotificationElem.classList.add('vjs-retro3-offline-notification')
