...
class Retro3HotkeysPlugin extends Plugin {
  ...
  private buildHandlers () {
    const handlers: KeyHandler[] = [
      ...
      // Increase volume
      {
        ...
        cb: e => {
          e.preventDefault()
          this.player.volume(this.player.volume() + Retro3HotkeysPlugin.VOLUME_STEP)
        }
      },
    ...
    const handlers: KeyHandler[] = [
      // Rewind
      {
        ...
        cb: e => {
          ...
          const target = Math.max(0, this.player.currentTime() - Retro3HotkeysPlugin.SEEK_STEP)
      ...
      // Forward
      {
        ...
        cb: e => {
          ...
          const target = Math.min(this.player.duration(), this.player.currentTime() + Retro3HotkeysPlugin.SEEK_STEP)
          this.player.currentTime(target)
        }
      }
    ]
...
videojs.registerPlugin('retro3HotkeysPlugin', Retro3HotkeysPlugin)
export { Retro3HotkeysPlugin }
